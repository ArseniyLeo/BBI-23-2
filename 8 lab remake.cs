...
class SurnameSortingTask : Task
{
    public override string Process(string input)
    {
        Console.WriteLine("Введите фамилии, разделенные запятой:");
        string surnames = Console.ReadLine();        
        string[] surnamesArray = surnames.Split(',');
        Array.Sort(surnamesArray, new SurnameComparator());
        return string.Join(", ", surnamesArray); 
    }
    public class SurnameComparator : IComparer<string>
    {
