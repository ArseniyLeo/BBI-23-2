...
class SurnameSortingTask : Task
{
    public override string Process(string input)
    {
        string[] Surname = input.Split(new char[] { ',', ' ', '.', '-', '(', ')' }, StringSplitOptions.RemoveEmptyEntries);
        ...
        for (int i = 1; i < Surname.Length; i++)
        {
            for (int j = 0; j < i; j++) 
            {
                ...
                if (string.Compare(Surname[i], Surname[j], StringComparison.OrdinalIgnoreCase) < 0)
                {
        ...
        string output = string.Join(" ", Surname);
        return output;
    }
}
...
class Program
{
    static void Main(string[] args)
    {
        ...
        Console.WriteLine("5) Вывести буквы, на которые начинаются слова в тексте, в порядке убывания их употребления.");
        Console.WriteLine("7) Вывести слова, содержащие заданную последовательность букв.");
        Console.WriteLine("11) Упорядочить список фамилий по алфавиту.");
        Console.WriteLine("14) Найти сумму чисел от 1 до 10 в тексте.");
